...
static const size_t INPUT_RING_BUFFER_SAMPLES = 96000;  // Audio samples
...
static const size_t TASK_DELAY_MS = 50;
...
esp_err_t AudioMixer::start(const std::string &task_name, UBaseType_t priority) {
  ...
  if (this->task_handle_ == nullptr) {
    this->task_handle_ =
        xTaskCreateStaticPinnedToCore(AudioMixer::audio_mixer_task_, task_name.c_str(), TASK_STACK_SIZE, (void *) this,
                                      priority, this->stack_buffer_, &this->task_stack_, tskNO_AFFINITY);
  }
...
void AudioMixer::audio_mixer_task_(void *params) {
  ...
  while (true) {
    if (xQueueReceive(this_mixer->command_queue_, &command_event, pdMS_TO_TICKS(0)) == pdTRUE) {
    ...
    if (combination_buffer_length > 0) {
      size_t output_bytes_written = this_mixer->output_ring_buffer_->write_without_replacement(
          (void *) combination_buffer, combination_buffer_length, pdMS_TO_TICKS(25));
      ...
      size_t output_free = OUTPUT_BUFFER_SAMPLES * sizeof(int16_t);
