...
class NabuMediaPlayer : public Component, public media_player::MediaPlayer {
 ...
 protected:
  ...
  bool set_stream_uri_(esp_http_client_handle_t *client, const std::string &new_uri);
  void cleanup_(esp_http_client_handle_t *client);
  // void cleanup_();

  std::unique_ptr<RingBuffer> output_ring_buffer_;

  QueueHandle_t http_read_event_queue_;
  QueueHandle_t http_read_command_queue_;

  TaskHandle_t http_read_task_handle_{nullptr};
  static void http_read_task_(void *params);
...
class HTTPStreamer : {
 public:
  ...
  /// @param length how many bytes to read from the ring buffer
  /// @return number of bytes actually read
  size_t read(uint8_t *buffer, size_t length) {
    size_t available_bytes = this->available();
    size_t bytes_to_read = std::min(length, available_bytes);
    return this->output_ring_buffer_->read((void *)available_bytes, bytes_to_read);
  }
  ...
  BaseType_t send_command(MediaCommandEvent &command, TickType_t ticks_to_wait = portMAX_DELAY) {
    xQueueSend(this->command_queue_, command, ticks_to_wait);
  }

  BaseType_t read_event(TaskEvent &event, TickType_t ticks_to_wait = 0) {
    xQueueReceive(this->event_queue_, event, ticks_to_wait);
  }
