      ...
      if (buffer == nullptr) {
        ...
        if (err != ESP_OK) {
          ...
          while (true) {
            ...
            bytes_read = this_speaker->audio_mixer_->read((uint8_t *) buffer, bytes_to_read, pdMS_TO_TICKS(10));
  ...
  if (type == AudioPipelineType::MEDIA) {
    ...
    if (url) {
      err = this->media_pipeline_->start(this->media_url_.value(), this->sample_rate_, "media",
                                         MEDIA_PIPELINE_TASK_PRIORITY, tskNO_AFFINITY);
    } else {
    ...
    if (url) {
      err = this->announcement_pipeline_->start(this->announcement_url_.value(), this->sample_rate_, "ann",
                                                ANNOUNCEMENT_PIPELINE_TASK_PRIORITY, tskNO_AFFINITY);
    } else {
