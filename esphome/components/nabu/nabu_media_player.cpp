...
esp_err_t NabuMediaPlayer::start_pipeline_(AudioPipelineType type, bool url) {
  ...
  if (type == AudioPipelineType::MEDIA) {
    ...
    if (this->is_paused_) {
      ...
      this->audio_mixer_->send_command_to_front(&command_event);
    }
...
void NabuMediaPlayer::set_ducking_ratio(float ducking_ratio, uint8_t steps) {
  if (this->audio_mixer_ != nullptr) {
    float starting_ratio = this->audio_mixer_->get_ducking_ratio();
    float ending_ratio = ducking_ratio;
    float factor_per_step = (ending_ratio - starting_ratio) / pow(2, steps);
    ...
    //   command_event.ducking_ratio = starting_ratio + pow(2,i)*factor_per_step;
    //   command_event.samples = 48000*2;
    //   this->audio_mixer_->send_command(&command_event);
    // }
  }
...
void NabuMediaPlayer::reconfigure_dac_new_settings_more_filters(){
		// Set register page to 0
		this->write_byte(AIC3204_PAGE_CTRL, 0x00);
    ...
    /*** MODIFICATIONHERE! ***
     * Use PRB_P3 processing block (see page 39)
     */
    // Program the DAC processing block to be used - PRB_P1
		this->write_byte(AIC3204_DAC_SIG_PROC, 0x03);
...
void NabuMediaPlayer::reconfigure_dac_new_settings_most_filters(){
		// Set register page to 0
		this->write_byte(AIC3204_PAGE_CTRL, 0x00);
    ...
    /*** MODIFICATIONHERE! ***
     * Use PRB_P24 processing block (see page 39)
     */
    // Program the DAC processing block to be used - PRB_P1
		this->write_byte(AIC3204_DAC_SIG_PROC, 0x18);
...
void NabuMediaPlayer::reconfigure_dac_new_settings_less_filters_alt(){
		// Set register page to 0
		this->write_byte(AIC3204_PAGE_CTRL, 0x00);
    ...
    /*** MODIFICATIONHERE! ***
     * Use PRB_P7 processing block (see page 39)
     */
    // Program the DAC processing block to be used - PRB_P1
		this->write_byte(AIC3204_DAC_SIG_PROC, 0x07);
		
    
    // Program the ADC processing block to be used - PRB_R1
        this->write_byte(AIC3204_ADC_SIG_PROC, 0x11);
		// Select Page 1
		this->write_byte(AIC3204_PAGE_CTRL, 0x01);
		// Enable the internal AVDD_LDO:
		this->write_byte(AIC3204_LDO_CTRL, 0x09);
...
void NabuMediaPlayer::reconfigure_dac_new_settings_less_filters(){
		// Set register page to 0
		this->write_byte(AIC3204_PAGE_CTRL, 0x00);
    ...
    /*** MODIFICATIONHERE! ***
     * Use PRB_P17 processing block (see page 39)
     */
    // Program the DAC processing block to be used - PRB_P1
		this->write_byte(AIC3204_DAC_SIG_PROC, 0x11);
