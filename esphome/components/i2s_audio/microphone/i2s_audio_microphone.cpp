...
size_t I2SAudioMicrophone::read(int16_t *buf, size_t len, TickType_t ticks_to_wait) {
  ...
  // size_t adjusted_length = len;
  // if (this->bits_per_sample_ == I2S_BITS_PER_SAMPLE_32BIT) {
  //   // len specifies how many bytes to read as int16 samples
  //   adjusted_length *= sizeof(int32_t) / sizeof(int16_t);
  // }

  // esp_err_t err = i2s_read(this->parent_->get_port(), buf, adjusted_length, &bytes_read, ticks_to_wait);
  ...
  switch (this->bits_per_sample_) {
    ...
    case I2S_BITS_PER_SAMPLE_32BIT: {
      ...
      for (size_t i = 0; i < samples_read; i++) {
        ...
        buf[i] = (int16_t) clamp<int32_t>(temp, INT16_MIN, INT16_MAX);
      }
...
size_t I2SAudioMicrophone::read(int16_t *buf, size_t len) { return this->read(buf, len, 100 / portTICK_PERIOD_MS); }
