    ...
    if (notification_bits & TaskNotificationBits::COMMAND_START) {
      ...
      if ((buffer == nullptr) || (channel_1_samples.capacity() < FRAMES_IN_ALL_DMA_BUFFERS) ||
        ...
        if (err != ESP_OK) {
          ...
          while (true) {
            notification_bits = ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(0));//TASK_DELAY_MS));
            ...
            esp_err_t err = i2s_read(this_microphone->parent_->get_port(), buffer,
                                     SAMPLES_IN_ALL_DMA_BUFFERS * sizeof(int32_t), &bytes_read, portMAX_DELAY);
            if (err != ESP_OK) {
              event.type = i2s_audio::TaskEventType::WARNING;
              event.err = err;
              xQueueSend(this_microphone->event_queue_, &event, portMAX_DELAY);
          ...
          allocator.deallocate(buffer, FRAMES_IN_ALL_DMA_BUFFERS);
