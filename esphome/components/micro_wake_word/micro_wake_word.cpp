...
void MicroWakeWord::preprocessor_task_(void *params) {
  ...
  while (true) {
    ...
    xEventGroupClearBits(this_mww->event_group_, EventGroupBits::PREPROCESSOR_MESSAGE_IDLE);
    {
      ...
      int16_t *audio_buffer = int16_allocator.allocate(new_samples_to_read * 2);
      ...
      while (!(xEventGroupGetBits(this_mww->event_group_) & COMMAND_STOP)) {
        ...
        if (bytes_read < 2 * new_samples_to_read * sizeof(int16_t)) {
          // This shouldn't ever happen, but if we somehow don't have enough samples, just drop this frame
          continue;
        }

        if ((this_mww->microphone_->available() + new_samples_to_read * sizeof(int16_t)) == 64 * 16 * sizeof(int16_t)) {
          ESP_LOGW(TAG, "Losing samples in microphone ring buffer");
        }
        ...
        for (int i = 0; i < 2; ++i) {
          ...
          memmove(audio_buffer, audio_buffer + num_samples_processed, new_samples_to_read * sizeof(int16_t));
...
void MicroWakeWord::start() {
  ...
  if (this->preprocessor_task_handle_ == nullptr) {
    this->preprocessor_task_handle_ =
        xTaskCreateStaticPinnedToCore(MicroWakeWord::preprocessor_task_, "preprocessor", PREPROCESSOR_TASK_STACK_SIZE,
                                      (void *) this, PREPROCESSOR_TASK_PRIORITY, this->preprocessor_task_stack_buffer_,
                                      &this->preprocessor_task_stack_, tskNO_AFFINITY);
  }

  if (this->inference_task_handle_ == nullptr) {
    this->inference_task_handle_ = xTaskCreateStaticPinnedToCore(
        MicroWakeWord::inference_task_, "inference", INFERENCE_TASK_STACK_SIZE, (void *) this, INFERENCE_TASK_PRIORITY,
        this->inference_task_stack_buffer_, &this->inference_task_stack_, tskNO_AFFINITY);
