...
bool ElevenLabsClient::get_signed_url(std::string& signed_url_out) {
  ...
  config.event_handler = [](esp_http_client_event_t *evt) -> esp_err_t {
    ...
    if (evt->event_id == HTTP_EVENT_ON_DATA && evt->data_len > 0) {
      resp->append(static_cast<const char*>(evt->data), evt->data_len);
    }
    return ESP_OK;
  };
  ...
  if (!api_key_.empty()) {
    esp_http_client_set_header(client, "xi-api-key", api_key_.c_str());
  }
  esp_task_wdt_reset();
  ...
  if (err == ESP_OK && status_code == 200 && !response.empty()) {
    // Parse JSON response for signed_url
    bool found = false;
    json::parse_json(response, [&signed_url_out, &found](JsonObject root) -> bool {
      const char* signed_url = root["signed_url"];
      if (signed_url) {
        signed_url_out = signed_url;
        found = true;
      }
      return found;
    });
  ...
  ESP_LOGE(TAG, "Failed to get signed URL: %s", response.c_str());
  return false;
}
...
bool ElevenLabsClient::connect(const std::string& signed_url,
  ...
  esp_err_t reg_err = esp_websocket_register_events(websocket_client_, WEBSOCKET_EVENT_ANY, &ElevenLabsClient::websocket_event_handler, this);
  if (reg_err != ESP_OK) {
...
void ElevenLabsClient::websocket_event_handler(void* handler_args, esp_event_base_t base, int32_t event_id, void* event_data) {
  ...
  switch (event_id) {
    case WEBSOCKET_EVENT_CONNECTED:
      ...
      if (client->on_connected_) client->on_connected_();
      break;
    case WEBSOCKET_EVENT_DISCONNECTED:
      client->websocket_connected_ = false;
      if (client->on_disconnected_) client->on_disconnected_();
      break;
    case WEBSOCKET_EVENT_DATA:
      if (data->op_code == 0x01 && data->data_len > 0) {
        if (client->on_message_) client->on_message_(data->data_ptr, data->data_len);
      }
      break;
    case WEBSOCKET_EVENT_ERROR:
      if (client->on_error_) client->on_error_("WebSocket connection error");
      break;
    default:
      break;
  }
